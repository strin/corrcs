function [reX code G] = gcs(B, mask, X, graph, W)
    %% parameters.
    beta0               = 1e2;
    param.iteration     = 200;
    param.diag          = 0;
    ...
    for iter = 1:param.iteration
        ...
        for ni = 1:num_node
            ...
            for nj = 1:num_node
              % strategy a. exact em.
%                 G(ni, nj) = 2 * kappa * (sum(signal_mu{ni}.*signal_mu{ni}+diag(signal_sigma{ni})+signal_mu{nj}.*signal_mu{nj}+diag(signal_sigma{nj}))...
%                                           - 2 * sum(signal_mu{ni} .* signal_mu{nj}));
              % strategy b. relax.
                G(ni, nj) = 2 * kappa * norm(signal_mu{ni} - signal_mu{nj})^2;
            end
        ...
        for gi = 1:param.gaussround
            for ni = 1:num_node
                ...
                for nj = 1:num_node
                    ...
                    pp_sigma = pp_sigma+G(ni, nj) * W;
                    pp_mu = pp_mu+G(ni, nj) * W * signal_mu{nj};
                end    
                if param.diag
