function [reX code] = gcs(B, mask, X, graph, W, b)
    %% parameters.
    beta0               = 1e2;
    param.iteration     = 100;
    param.diag          = 0;
...
%               beta(ni)  = (length(y{ni})-sum(gamma))/norm(y{ni}-B(mask(:,ni),:)*signal_mu{ni})^2;
        ...
        % E-step. estimate PG variable.
        graph_pg = zeros(size(graph));
        for i = 1:length(edgeI)
            ei = edgeI(i); ej = edgeJ(i);
            graph_pg(ei, ej) = pgep(param.c, sqrt(trace(signal_corr{ei}*W*signal_corr{ej}*W')...
                                            +2*b*signal_mu{ei}'*W*signal_mu{ej}+b^2), param.pgsample);
        end
        ...
        for gi = 1:param.gaussround
            for ni = 1:num_node
                ...
                for nj = 1:num_node
                    ...
                    pp_sigma = pp_sigma+graph_pg(ni, nj)*W*signal_corr{nj}*W';
                    pp_mu = pp_mu+(1/2-b*graph_pg(ni,nj))*W*signal_mu{nj};
